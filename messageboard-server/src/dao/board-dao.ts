...
export async function getOneMessage(messageId: any): Promise<Message> {
  ...
  try {
    ...
    logger.debug(
      `result message from getOneMessage ${result.rows[0].messageId}`
    );
...
export async function postSerbianMessage(
  newMessage: Message
): Promise<Message> {
  let client: PoolClient;
  ...
  try {
    ...
    let result = await client.query(
      `insert into messageboard.foreign_messages("user_id", "title", "message")
                                            values ($1, $2, $3)
                                            returning message_id;`,
      [newMessage.userId, newMessage.title, newMessage.message]
    );
